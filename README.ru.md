...
- **Режим статей и лонгридов Telegraph (`/article`):** автоматическая верстка подробных статей и черновиков на платформе Telegraph с публикацией анонса в Telegram-канал и поддержкой нативного режима **Instant View**.
- **🎨 Автогенерация авторских AI-обложек (FLUX.1):** если при публикации статьи не прикреплено изображение, ИИ сам составляет точный англоязычный промпт и генерирует уникальную обложку 16:9 высокого разрешения через нейросеть FLUX.
- **🏷 Умные навигационные теги:** автоматическое извлечение и публикация 3–6 релевантных кликабельных хештегов в конце статьи и в посте-анонсе.
- **Сворачиваемые цитаты Telegram (`<blockquote expandable>`):** упаковка длинных списков, параметров и цитат в аккуратные сворачиваемые блоки Telegram (поддерживается синтаксис `**>` и `> [!expandable]`).
...
- **Публикация постов (через ЛС):** владелец может отправлять боту в ЛС команду `/post <текст в формате Markdown>` (с опциональным прикреплением фото) для мгновенной публикации красивых Rich-постов в канал. Медиа сохраняется в `public/media` сайта на Vercel и публикуется по постоянному URL. Тексты длиннее 3800 символов автоматически публикуются как статья.
...
- **Команды владельца:** `/article`, `/post`, `/postlink`, `/ephemeral`, `/status`, `/usage`, `/on`, `/off`, `/chatid`.
...
![Панель управления](assets/admin_panel_ru.png)
...
- `/article <текст>` - сверстать и опубликовать лонгрид на Telegraph с Instant View, AI-обложкой FLUX и тегами.
- `/articleraw <текст>` - опубликовать статью на Telegraph без предварительного ИИ-редактирования текста.
