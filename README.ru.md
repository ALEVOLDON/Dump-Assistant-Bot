...
- **🎨 Фирменные AI-обложки (Google Gemini & FLUX):** если при публикации статьи или ссылки не прикреплено изображение, ИИ составляет точный промпт и генерирует уникальную обложку 16:9 в фирменном стиле канала DUMP (палитры Cyber, Solar, Emerald, Void) через **Google Gemini** (`gemini-2.5-flash-image`) с каскадным fallback на Pollinations FLUX. Строгий фильтр артефактов исключает текст и надписи на изображениях.
...
- `CHANNEL_USERNAME` - юзернейм канала без `@` для ссылок онбординга (по умолчанию `dump_dump`).
- `WEBAPP_URL` - публичный URL Mini App админки (устанавливает кнопку меню только для владельцев).
...
### 4. Панель управления (Admin Panel) и скрытие от гостей

У бота есть веб-панель управления (Telegram Web App / Mini App), которая позволяет настраивать параметры ИИ, смотреть статистику использования токенов и создавать/публиковать посты во вкладке «Постинг».

![Панель управления](assets/admin_panel_ru.png)

#### Защита интерфейса и персональная кнопка меню:
* **Гости никогда не видят кнопку админки**: при старте бота вызов Telegram API `setChatMenuButton` сбрасывает кнопку по умолчанию для всех обычных пользователей.
* **Владельцы видят кнопку Web App**: бот персонально настраивает кнопку меню Telegram (`web_app`) исключительно для Telegram ID из `OWNER_USER_IDS`, если в `.env` указан `WEBAPP_URL`.
* **Быстрый доступ по `/start`**: в личных сообщениях владельца команда `/start` также присылает inline-кнопку «🛠 Панель управления» для открытия админки в один клик.
...
3. Запустите туннель для порта бота (по умолчанию `3001`):
   ...
   ```
4. Укажите полученную ссылку в `.env`:
   ```env
   WEBAPP_URL=https://your-domain.ngrok-free.app/app
   ```
   При запуске бот сам зарегистрирует кнопку админки персонально для владельцев.
...
### 5. Онбординг в ЛС (Гости vs Владельцы)

* **Для гостей (читателей):**
  - По команде `/start` открывается интерактивное приветствие с кнопками:
    - `ℹ️ Что умеет бот` — объяснение функций (ответы в комментариях, ссылки, связь с админом).
    - `📢 Открыть канал` — прямая ссылка на канал.
    - `❓ Задать вопрос` — инструкция о том, что ИИ отвечает в комментариях под публикациями.
    - `✍️ Написать админу` — активация режима отправки сообщения создателю канала.
  - **Без спама владельцу**: нажатия на инфо-кнопки и повторные `/start` не отправляют никаких уведомлений владельцу.
  - **Связь с автором (Relay)**: если гость нажимает «Написать админу» и пишет сообщение, оно пересылается владельцу. Владелец может ответить гостю простым Telegram Reply на уведомление.
  - **Аудит содержательных сообщений**: если гость пишет свободный текст в ЛС, бот подсказывает перейти в комментарии канала, а владельцу отправляет компактный лог аудита (`[Audit DM]`) с защитой от флуда (cooldown 60 секунд). Владелец может ответить на сообщение аудита через Reply.

* **Рекомендации для BotFather (описание бота):**
  - **About:** Официальный AI-ассистент канала @dump_dump. Разбор публикаций, ответы на вопросы в комментариях и связь с автором.
  - **Description:** Привет! Я AI-ассистент канала @dump_dump. Отвечаю на технические вопросы в обсуждениях к публикациям, делаю выжимки ссылок и передаю вопросы создателю. Откройте канал или нажмите /start!
...
- `/article <текст>` - сверстать и опубликовать лонгрид на Telegraph с Instant View, брендовой AI-обложкой Gemini и тегами.
