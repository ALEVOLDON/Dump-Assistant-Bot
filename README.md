...
- **Нативное форматирование (Telegram Bot API 10.1):** бот поддерживает отправку Rich Messages (нативные таблицы, списки, LaTeX-формулы) через конвертацию Markdown в HTML с помощью библиотеки `marked`. Реализована автоматическая защита и откат (fallback) на стандартные сообщения для старых клиентов Telegram.
...
- **Публикация постов (через ЛС):** владелец может отправлять боту в ЛС команду `/post <текст в формате Markdown>` (с опциональным прикреплением фото) для мгновенной публикации красивых Rich-постов в канал. Бот автоматически зальет картинку на хостинг и вернет прямую ссылку на публикацию.
- **Команды владельца:** `/post`, `/status`, `/usage`, `/on`, `/off`, `/chatid`.
