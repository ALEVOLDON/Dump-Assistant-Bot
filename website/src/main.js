...
const translations = {
  en: {
    ...
    faq_a6: 'As an owner, send `/post <Markdown text>` to the bot in a direct message. You can optionally attach an image, which the bot will automatically host and format into a native Telegram Rich Message. The bot will respond with a direct link to the post.',
  ...
  ru: {
    ...
    faq_a6: 'Владелец может отправить команду `/post <Markdown-текст>` в личные сообщения бота. К сообщению можно прикрепить картинку — бот автоматически загрузит её на хостинг и сформирует красивое единое Rich-сообщение. После публикации бот пришлет прямую ссылку на пост.',
