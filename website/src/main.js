...
const translations = {
  en: {
    ...
    features_rich_desc: 'Supports Telegram Bot API 10.2+ native tables, lists, LaTeX formulas, and ephemeral group replies with automatic fallback.',
    ...
    features_owner_title: 'Owner Co-pilot & Scoped UI',
    features_owner_desc: 'Scoped admin UI hidden from guests, rich channel posting, Telegraph longreads, and compact visitor DM audit notifications.',
    ...
    commands_start_desc: 'Guest onboarding / Owner dashboard',
    commands_post_desc: 'Publish markdown post to channel',
    commands_article_desc: 'Publish article on Telegraph (Instant View)',
    ...
    commands_ephemeral_desc: 'Toggle ephemeral replies in groups (on/off)',
    ...
    faq_q7: 'How do I access the Web App admin panel and is it secure from guests?',
    faq_a7: 'Start the bot and ngrok tunnel (via `start.bat` on Windows). The bot automatically scopes the Telegram menu button exclusively to owners listed in `OWNER_USER_IDS` via `WEBAPP_URL` — guests never see the admin button. Owners can also access the panel directly via `/start` in DM. For local browser testing, set `BYPASS_INIT_DATA_AUTH=true` in `.env`.',
  ...
  ru: {
    ...
    features_rich_desc: 'Поддержка Telegram Bot API 10.2+ (таблицы, списки, формулы LaTeX) и приватных ответов в группах с автоматическим откатом.',
    ...
    features_owner_title: 'Панель владельца и защита UI',
    features_owner_desc: 'Защищённая админка (скрыта от гостей), публикация постов и лонгридов Telegraph, а также компактный аудит обращений гостей в ЛС.',
    ...
    commands_start_desc: 'Онбординг гостей / Панель владельца',
    commands_post_desc: 'Опубликовать Markdown-пост в канал',
    commands_article_desc: 'Опубликовать статью на Telegraph (Instant View)',
    ...
    commands_ephemeral_desc: 'Вкл/выкл приватные ответы бота в группах (on/off)',
    ...
    faq_q7: 'Как запустить веб-панель управления и защищена ли она от гостей?',
    faq_a7: 'Запустите бота и туннель ngrok (через `start.bat` на Windows). Бот автоматически настраивает кнопку меню Telegram исключительно для владельцев из `OWNER_USER_IDS` через `WEBAPP_URL` — для обычных гостей кнопка админки скрыта. Владельцы также получают прямую кнопку открытия панели по команде `/start`. Для тестов в обычном браузере используйте `BYPASS_INIT_DATA_AUTH=true`.',
